...
<head>
    ...
    <title>Dementia Monitoring App carer reset password page</title>
...
if(isset($_POST['user_submit'])){
    if (isset($_POST['username'])) {
        ...
        $reset_code = generateRandomString();
        $_SESSION['reset_code'] = $reset_code;

        $query = "SELECT * FROM Carers WHERE username ='".$_SESSION['reset_user']."'";
        $result = $link->query($query);

        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $to = $row['email'];
                $subject = 'Dementia Monitoring App';
                $message = "This is the code to reset your password: ".$_SESSION['reset_code']."";
...
if (isset($_POST['reset_submit'])){
    ...
    if(empty($password_error) && empty($confirm_password_error) && empty($resetCode_error)){
        ...
        $query = "UPDATE `Carers` SET password = '".md5($password)."' WHERE username = '".$_SESSION['reset_user']."'";
        $result = mysqli_query($link,$query);

        if($result){
            header("location: carer-logout.php");
        }
        else{
...
<main>
    <form class="reset-password" action="carer-email-password.php" method="post" >
        <div class="reset-password-content">
            ...
            <div class="reset-password-links">
                <a class="reset-password-links" href="carer-login.php">Cancel</a>
            </div>
        </div>
    </form>
</main>
